...
import io.thoughtware.gittok.common.CronUtils;
import io.thoughtware.gittok.common.GitTokFinal;
import io.thoughtware.gittok.timedtask.dao.TimeTaskInstanceDao;
import io.thoughtware.gittok.timedtask.entity.TimeTaskInstanceEntity;
import io.thoughtware.gittok.timedtask.model.TimeTaskInstance;
import io.thoughtware.gittok.timedtask.model.TimeTaskInstanceQuery;
...
public class TimeTaskInstanceServiceImpl implements TimeTaskInstanceService {
    ...
    public String createTimeTaskInstance(@NotNull @Valid TimeTaskInstance timeTaskInstance) {
        ...
        try {
            jobManager.addJob(timeTaskInstance, RunJob.class, GitTokFinal.DEFAULT);
        ...
        if (("timeTaskId").equals(fieldType)){
            ...
            if (CollectionUtils.isNotEmpty(timeTaskInstanceList)){
                for (TimeTaskInstance taskInstance:timeTaskInstanceList){
                    jobManager.removeJob(GitTokFinal.DEFAULT,taskInstance.getId()+"_"+taskInstance.getExecObjectId());
                }
            }
        }
