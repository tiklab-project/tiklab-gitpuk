package io.thoughtware.gittok.statistics.service;
...
import io.thoughtware.gittok.repository.service.RecordCommitServiceImpl;
import io.thoughtware.gittok.repository.service.RepositoryService;
import io.thoughtware.gittok.statistics.common.GitCommit;
import io.thoughtware.gittok.statistics.model.RepositoryStatistics;
import io.thoughtware.gittok.statistics.model.Statistics;
import io.thoughtware.gittok.statistics.model.StatisticsQuery;
import io.thoughtware.gittok.statistics.model.UserStatistics;
...
public class StatisticsServiceImpl implements StatisticsService{
    ...
    public Statistics commitRpyUserStatistics(StatisticsQuery statisticsQuery) {
        ...
        //查询所有仓库
        List<io.thoughtware.gittok.repository.model.Repository> rpyList = repositoryService.findRpyList();
    ...
    public Statistics commitRpyStatistics(StatisticsQuery statisticsQuery) {
        ...
        if (CollectionUtils.isEmpty(commitList)){
            //查询时间段内没有提交的合并请求
            List<io.thoughtware.gittok.repository.model.Repository> repositoryList = findRepositoryList(rpyList, findNum);
            ...
            List<RepositoryStatistics> statistics = rpyStatisticsList.stream()
                    .sorted(Comparator.comparing(RepositoryStatistics::getCommitCount).reversed())
                    .collect(Collectors.toList());
            if (statistics.size()>findNum){
                statistics = statistics.subList(0, findNum);
            }
    ...
    public Statistics mergeReqRpyStatistics(StatisticsQuery statisticsQuery) {
        ...
        if (CollectionUtils.isEmpty(mergeRequestList)){
            ...
            //查询时间段内有提交的合并请求
            for (io.thoughtware.gittok.repository.model.Repository repo : rpyList){
 ...
 /*       if(!org.springframework.util.StringUtils.isEmpty(projectSetId)){
     ...
     * @param cutOutNum 截取数
     */
    public List<io.thoughtware.gittok.repository.model.Repository> findRepositoryList(List<io.thoughtware.gittok.repository.model.Repository> repositoryList,Integer cutOutNum ){
        List<io.thoughtware.gittok.repository.model.Repository> repositorys=repositoryList;
