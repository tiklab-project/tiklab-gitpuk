package io.tiklab.gitpuk.timedtask.service;
...
import io.tiklab.gitpuk.timedtask.model.TimeTaskInstanceQuery;
import io.tiklab.gitpuk.common.CronUtils;
import io.tiklab.gitpuk.common.GitPukFinal;
import io.tiklab.gitpuk.timedtask.dao.TimeTaskInstanceDao;
import io.tiklab.gitpuk.timedtask.entity.TimeTaskInstanceEntity;
...
public class TimeTaskInstanceServiceImpl implements TimeTaskInstanceService {
    ...
    public String createTimeTaskInstance(@NotNull @Valid TimeTaskInstance timeTaskInstance) {
        ...
        if (CollectionUtils.isNotEmpty(timeTaskInstanceList)){
            throw new ApplicationException(GitPukFinal.TIME_EXCEPTION,"当前时间已经添加过，无需重复添加。");
        }
        ...
        try {
            jobManager.addJob(timeTaskInstance, RunJob.class, GitPukFinal.DEFAULT);
        ...
        if (("timeTaskId").equals(fieldType)){
            ...
            if (CollectionUtils.isNotEmpty(timeTaskInstanceList)){
                for (TimeTaskInstance taskInstance:timeTaskInstanceList){
                    jobManager.removeJob(GitPukFinal.DEFAULT,taskInstance.getId()+"_"+taskInstance.getExecObjectId());
                }
            }
        }
