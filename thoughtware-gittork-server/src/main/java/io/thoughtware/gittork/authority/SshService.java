package io.thoughtware.gittork.authority;


import io.thoughtware.gittork.common.RepositoryFinal;
...
import io.thoughtware.gittork.setting.service.AuthServer;
...
public class SshService {
    ...
    @Value("${gittork.ssh.port:10000}")
    private int sshPort;

    @Value("${gittork.ssh.key:/conf/id_rsa}")
    private String sshKey;

    @Autowired
    GitTorkYamlDataMaService GitTorkYamlDataMaService;
    ...
    public  void sshAuthority()  {
      ...
      /*  if (!property.endsWith(RepositoryFinal.APP_NAME)){
        ...
        try {
            sshServer.setCommandFactory(new GitTorkSshCommandFactory(GitTorkYamlDataMaService,repositoryServer));
            sshServer.start();
     ...
     * 通过不同请求触发不同钩子
     */
    private static class GitTorkSshCommandFactory implements CommandFactory {
        private static Logger logger = LoggerFactory.getLogger(GitTorkSshCommandFactory.class);

        private final GitTorkYamlDataMaService yamlDataMaService;

        private final RepositoryServer repositoryServer;

        public GitTorkSshCommandFactory(GitTorkYamlDataMaService yamlDataMaService, RepositoryServer repositoryServer) {
        ...
        public Command createCommand(ChannelSession channelSession, String command) {
           ...
           if (s.startsWith("/")){
            ...
            io.thoughtware.gittork.repository.model.Repository repositoryByAddress = repositoryServer.findRepositoryByAddress(s);
